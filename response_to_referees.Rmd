...
have extended our discussion about why we think the field would benefit
...
> We agree that a better contextualization will help us to show how widely this
...
from 2007 until today. We believe that this table illustrates how broadly this
...
to the API itself, meaning that it may change in future releases. However,
 we have organized the code so that all calls to ODE.jl go through
a single function, it will therefore be easy for us to track API changes. In
over a year of working with ODE.jl, we have noted no change in the API (and no
...
At present, though, I don’t think the authors make a compelling case that
 ...
 ecology, and helped produce some influential results.  Each of the studies
 ...
 type of community, and that it can be used in combination with other models
 ...
 did not result in publications, we believe that this type of package is
...
we now specify in the documentation that the recommended way to install Julia
...
ecology and, as can be seen in the SUpp. Mat. table, is widely used in the
...
both and corrected all mistakes we could find.
...
>The `foodweb_diversity` function indeed refers to the evenness component of
the Shannon index. We agree with the reviewer in that the name we had chosen
could have been confusing for users and we followed his suggestion. We thus
renamed this function `foodweb_evenness`. The modification will only appear
in the `0.2.0` release of the package.
...
and *exclusion* in the figure, which succintly represent the outcomes.
...
>The `species_persistence` function is now documented, and a bug in it
has been fixed (in release `0.1.1`). The documentation can be accessed
by typing `?species_persistence` in Julia. In addition, we have added
`species_richness` as an output function (which avoids the problems identified
by the reviewer). We also changed the code in part IV.iii, we now use the
function `species_persistence` instead of dividing by the initial richness. We
thank the reviewer for suggesting this improvement.
