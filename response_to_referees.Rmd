...
model has been used and still is. We have included a table in supp. mat.
...
At present, though, I don’t think the authors make a compelling case that
 ...
 >As shown in table \ref{listpapers} (now included as supp. mat.), Yodzis & Ines (1990) bio-energetic model has been widely used to
...
I have downloaded Julia and the befwm package, and tried some of the
 ...
 > We also thank the reviewer
 for pointing at the inconsistencies between the code presentented in the
 manuscript and the tutorials, we have been through both and corrected the
 mistakes we could find.
...
1. the measure referred to as `foodweb_diversity` (P8 L159) is identical to the
 ...
 evenness component of the Shannon index, as noted by the reviewer. we believe
 that the use of this index is a good -- and broadly used -- indicator of
 diversity within communities, here accounting for the number of remaining
 species as well as their relative abundance -- which perfectly fit the
 definition of diversity. We understand the view of the reviewer, although we
 prefer that the name of the function actually refers to what it is measuring
 instead of the name of the index itself. We are confident that the users --
 mostly food-web ecologists -- will perfectly understand what the
 `foodweb_diversity` function does by reading its documentation, available by
 simply typing `?foodweb_diversity`.  
...
2. in IV.i and figure 1, I suggest using outcome-neutral terminology in place
 ...
 values of relative inter-specific competition. We have replaced it in the text
 following the suggestions of the reviewer. We now refer to inter-specific
 competition greater, equal or lower than intra-specific competition. Although
 we kept the terms *coexistence*, *neutrally stable* and *exclusion* in the
 figure, which represent the outcomes.
